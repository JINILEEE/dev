...
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 첫번째 인자에서 두번째 인자를 탐색 B의 위치값 출력
SELECT INSTR('AABAACAABBAA', 'B', 4) FROM DUAL; -- 3번째 인자값 인덱스 부터 탐색 시작
...
/*
    3) LPAD / RPAD
        - LPAD/RPAD(컬럼|'문자값', 길이(글자수)[, '덧붙이려고 하는 문자'])
...
SELECT EMP_NAME, BONUS , SALARY * 12 + SALARY * NVL(BONUS,0) AS "보너스가 포함된 연봉"
FROM EMPLOYEE
;

-- EMPLOYEE 테이블에서 사원명, 부서 코드 조회 (단, 부서코드가 NULL 이면 "부서없음" 출력)
SELECT EMP_NAME, NVL(DEPT_CODE,'부서없음') AS "부서 코드"
FROM EMPLOYEE;

--보너스 일괄적용 하여 연봉 조회(무조건 10%로 적용, 없으면 안줌)
--사원 이름, 보너스, 연봉, 월급, 보너스받은금액
SELECT EMP_NAME, BONUS, SALARY * 12 + SALARY * NVL2(BONUS,0.1,0)  "연봉", SALARY, SALARY * NVL2(BONUS,0.1,0) AS "보너스 받은 금액"
FROM EMPLOYEE;
...
SELECT '123' AS ZZZZZZZZZZZZZZZ FROM DUAL;
...
--모든 사원 중에서 급여가 가장 많은 사람과 적은 사람
SELECT MAX(SALARY)AS 최대급여, MIN(SALARY) 최소급여, SUM(SALARY), AVG(SALARY)
FROM EMPLOYEE;
